-- Publication is one-way, and the database is what says so (Phase 4, decision 2).
--
-- The Phase 0 trigger already refused to unpublish a signal and to re-date one
-- that was published. It did not refuse the two remaining ways the pair could
-- drift: stamping `published_at` on a row that is still unpublished, and (by
-- omission) any future path that touches the timestamp without flipping the
-- flag. Both are closed here, so the only legal change to either column is the
-- single statement `published false -> true` with `published_at null -> set`.
--
-- Same function name, redefined in place, so the existing trigger binding on
-- `signals` picks up the new body without being recreated. Same SQLSTATE
-- 'OL001' as every other immutability breach.
