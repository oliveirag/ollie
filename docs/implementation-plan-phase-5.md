...
- [x] 5.5 **iOS.** *(`LiveApprovalUITests` drives it against the orchestrator started with `BROKER=mock` — fixture broker, orders fill on the first poll, refused in production. Verified 2026-09-15: sheet → acknowledgement → order placed → poll filled it → live execution, open lot, published. The live sheet opens full height so its confirm button is never below the fold.)* The live confirmation sheet; order-placed / awaiting-fill / cancelled states on the approvals list and detail; the autonomy section under Controls (locked while the env gate is off, mirroring the mode toggle); `live_since` on both Record screens as a visible boundary on the curve. **Exit:** on the simulator against the local backend in live mode with the mock broker, approving shows the red sheet, requires the second tap, and lands on "order placed"; XCUITest-driven.
